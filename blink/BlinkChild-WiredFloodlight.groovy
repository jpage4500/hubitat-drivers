/*
* BlinkChild-WiredFloodlight
*
* Description:
* This Hubitat driver provides a spot to put data from Blink Wired Floodlight (Superior owls)
*
...
* Ability to Arm/Disarm the system the Wired Floodlight is on
...
* 
* KNOWN ISSUE(S):
* Wired Floodlight (Superior Owls) have a set of limitations. It is not known if these are device or API limitations:
...
*
* Version Control:
* 0.1.6 - Correction to remove Driver Status when preferences are saved
* 0.1.5 - Correction to ProcessEvent function and removal of old driver-specific attributes when Preferences are saved
* 0.1.4 - Attempts to capture a new video clip for camera
* 0.1.3 - Changes for getting thumbnails
* 0.1.2 - Changes to how the light on/off is attempted
* 0.1.1 - Changes to how the light on/off is attempted
* 0.1.0 - Initial version based on MiniCamera driver
* 
* Thank you(s):
...
def DriverName(){
    return "BlinkChild-WiredFloodlight"
}
...
// Driver Metadata
metadata{
	definition( name: "BlinkChild-WiredFloodlight", namespace: "Snell", author: "David Snell", importUrl: "https://www.drdsnell.com/projects/hubitat/drivers/BlinkChild-WiredFloodlight.groovy" ) {
        capability "Sensor"
        capability "Actuator"
        capability "Switch"
        ...
        // Commands
        command "on", [ [ name: "This will Arm system Wired Floodlight is part of" ] ]  // 
        command "off", [ [ name: "This will Disarm system Wired Floodlight is part of" ] ]  // 
        command "FloodlightOn", [ [ name: "This will attempt to turn on the floodlight lights" ] ]  // 
        command "FloodlightOff", [ [ name: "This will attempt to turn off the floodlight lights" ] ]  // 
        command "SystemArm", [ [ name: "This will Arm system Wired Floodlight is part of" ] ]  // 
        command "SystemDisarm", [ [ name: "This will Disarm system Wired Floodlight is part of" ] ]  // 
        //command "MotionDetectionEnable", [ [ name: "This will Enable motion detection for Camera" ] ] // 
        //command "MotionDetectionDisable", [ [ name: "This will Disable motion detection for Camera" ] ] // 
...
def GetCameraLiveView(){
    ...
    Temp = Temp.split( "-" )
    Logging( "GetCameraLiveView on Camera ${ Temp[ 1 ] }.", 4 )
...
def DisableMotionDetection(){
    def Temp = device.getDeviceNetworkId()
    Temp = Temp.split( "-" )
    Logging( "Disabling Motion Detection on Camera-${ Temp[ 1 ] }.", 4 )
...
// FloodlightOn command with this device
def FloodlightOn(){
    if( ( state."Network ID" != null ) && ( state.ID != null ) ){
        parent.WiredFloodlightOn( state."Network ID", state.ID )
    } else {
        Logging( "Device does not know the Network ID = ${ state."Network ID" } or ID = ${ state.ID } to turn on floodlight", 5 )
    }
}

// FloodlightOff command with this device
def FloodlightOff(){
    if( ( state."Network ID" != null ) && ( state.ID != null ) ){
        parent.WiredFloodlightOff( state."Network ID", state.ID )
    } else {
        Logging( "Device does not know the Network ID = ${ state."Network ID" } or ID = ${ state.ID } to turn off floodlight", 5 )
    }
}
