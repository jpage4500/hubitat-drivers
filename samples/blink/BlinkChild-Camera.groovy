...
*
* Description:
* This Hubitat driver provides a spot to put data from Blink Cameras like the XT
*
...
* 0.1.4 - Changes for thumbnail capture
* 0.1.3 - Fix for "off" not working
* 0.1.2 - Adding ImageCapture capability to try to get thumbnails on device, right now it only takes a new thumbnail (same as GetNewThumbnail)
...
// Driver Metadata
metadata{
	definition( name: "BlinkChild-Camera", namespace: "Snell", author: "David Snell", importUrl: "https://www.drdsnell.com/projects/hubitat/drivers/BlinkChild-Camera.groovy" ) {
        capability "Sensor"
        capability "Battery"
        ...
        // Commands
        command "on", [ [ name: "This will Enable motion detection for Camera" ] ] // 
        command "off", [ [ name: "This will Disable motion detection for Camera" ] ] // 
        command "SystemArm", [ [ name: "This will Arm system Camera is part of" ] ] // 
        command "SystemDisarm", [ [ name: "This will Disarm system Camera is part of" ] ] // 
...
def GetCameraInfo(){
    ...
    if( state."Network ID" != null ){
        Logging( "GetCameraInfo on Camera-${ Temp[ 1 ] }.", 4 )
        parent.GetCameraInfo( Temp[ 1 ] )
    } else {
        Logging( "Camera-${ Temp[ 1 ] } does not know Network it belongs to, cannot request info.", 5 )
    }
}
