...
- **`sEpoch.seconds` wrong epoch calculation (DRIVER-BUG-3).** `use(TimeCategory) { new Date(0) + sEpoch.seconds }` treated the Unix timestamp as a duration, producing dates ~55,000 years in the future. Fixed: `new Date(sEpoch * 1000L)`. *(Note: claimed fixed in `41f8549` but the change was never actually applied; corrected in the subsequent commit.)*
